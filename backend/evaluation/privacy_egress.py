...
class _RemoteProvider(LLMProvider):
    ...
    async def health_check(self) -> JsonObject:
        return {"status": "ok", "provider": "remote-test", "local": False}
...
def _local_settings(port: int) -> Settings:
    return Settings(
        ...
        local_llm_base_url=f"http://127.0.0.1:{port}",
        local_llm_model_id="privacy-test-model",
        local_llm_request_timeout_seconds=2.0,
    )
...
def _remote_provider_rejected() -> bool:
    ...
    application = create_app(settings, _RemoteProvider())
    try:
...
def run_check() -> dict[str, Any]:
    ...
    with _server(_TrapHandler) as trap_server, _server(_ModelHandler) as model_server:
        ...
        provider = LocalOpenAIProvider(_local_settings(model_port))
        ...
        try:
            Settings(
                ...
                llm_provider="local_openai",
                privacy_mode="local_strict",
                local_llm_base_url="http://203.0.113.10:8000",
            )
        except ValueError:
